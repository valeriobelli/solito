...
```tsx twoslash
import { createUseParams } from 'solito'

const { useParams } = createParams()
```

`useParams` is a hook that lets you read the whole set of screen parameters on both Next.js and React Native. On Native, it reads React Navigation params, and on Web, it reads query params from `next/router`.

`useParams` reads both query parameters and dynamic route parameters. A Next.js dynamic route might look like `/artists/[slug].tsx`, where `slug` is a dynamic route. `useParam('slug')` works there too. On the native side, your linking config would have a URL with `/artists/:slug`.
...
You'll use `useParams()` instead of `useRoute().params` from React Navigation, and instead of `useRouter().query` from Next.js.
...
const { useParams } = createUseParams<Query>()
